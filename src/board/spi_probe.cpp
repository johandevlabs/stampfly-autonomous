...
static uint8_t spi_read_bmi270_reg(int cs, uint8_t reg, const SPISettings& s)
{
  uint8_t v;
  SPI.beginTransaction(s);
  digitalWrite(cs, LOW);
  ...
  SPI.transfer(0x00);        // DUMMY byte required by BMI270
  v = SPI.transfer(0x00);    // actual data
...
void spi_probe_devices()
{
  ...
  uint8_t bmi_id = spi_read_bmi270_reg(PIN_CS_BMI270, 0x00, SPI_BMI270_SETTINGS);
  Serial.printf("BMI270 probe (MODE0 @1MHz, READ+DUMMY): 0x%02X\n", bmi_id);
